...
MPI_COMM_WORLD);}
            ...
            for (int i = 1; i < size; i++) {
                MPI_Recv(&Y[(int)i * N1 / size], (int)N1 / size, MPI_FLOAT, i, 0, 
MPI_COMM_WORLD, &status);
            }
        }
        time2 = MPI_Wtime(); // Определение времени конца обработки
        double time = time2 - time1;
        for (int i = 0; i < N1; i++) { // Вывод результатов
            printf("%f\n", Y[i]);
        }
        printf("TIME=%f\n", time);
    }
        ...
        // Для всех процессов, кроме нулевого
        for (int cicl = 0; cicl < 10000; cicl++) { // Цикл кратности
            // Прием массивов
            MPI_Recv(&A[(int)rank * N1 / size], (int)N1 / size,
                MPI_FLOAT, 0, 0, MPI_COMM_WORLD, &status);
            // Вычисления
            for (int i = rank * N1 / size; i < (rank + 1) * N1 / size; i++) {
                Y[i] = (A[i] + B[i] * S1 + C[i]) * A[i];
            }
